...
function createForecastWeatherIcon(data, forecastIntemDiv) {
    let newIcon =$("<img>").attr("src", data);
    $(forecastIntemDiv).append(newIcon);
}
    ...
    $.ajax({
     ...
     $.ajax({
         ...
         for (let a = 0; a < 5; a++) {
             ...
             let forecastData = {
                 "temp": forecastInfo.daily[a].temp.day,
                 "humidity": forecastInfo.daily[a].humidity,
                 "icon": forecastInfor.daily[a].weather[0].icon,
         ...
         createUVEl(forecastInfo.current.uvi);
     });
